...
/**
 * Full pipeline untuk EPUB terenkripsi LCP.
 *
 * LCP EPUB: setiap resource di dalam ZIP di-enkripsi secara individual.
 * Alur:
 *   1. Buka EPUB sebagai ZIP
 *   2. Baca META-INF/encryption.xml → daftar file terenkripsi
 *   3. Untuk setiap file terenkripsi: IV(16) || ciphertext → decrypt → plaintext
 *   4. Rekonstruksi ZIP tanpa encryption.xml
 *   5. Kembalikan bytes ZIP baru ke epubjs
 */
export async function decryptEpub(
  ...
  // Baca daftar file yang terenkripsi dari encryption.xml
  const encXmlEntry = zip.file('META-INF/encryption.xml');
  if (!encXmlEntry) {
    // Tidak ada encryption.xml — EPUB tidak terenkripsi, kembalikan apa adanya
    return new Uint8Array(epubBuffer);
  }

  const encXmlText = await encXmlEntry.async('text');
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(encXmlText, 'application/xml');

  // querySelectorAll tidak menangani namespace prefix (enc:CipherReference).
  // Gunakan getElementsByTagName('*') + filter localName agar namespace-agnostic.
  const encryptedPaths = new Set<string>();
  for (const el of Array.from(xmlDoc.getElementsByTagName('*'))) {
    if (el.localName === 'CipherReference') {
      const uri = el.getAttribute('URI');
      if (uri) encryptedPaths.add(decodeURIComponent(uri));
    }
  }

  // Rekonstruksi ZIP: dekripsi file yang terdaftar, salin sisanya
  const outZip = new JSZip();
  for (const [filePath, entry] of Object.entries(zip.files)) {
    if (entry.dir) continue;
    // Hapus encryption.xml dari output — tidak dibutuhkan oleh epubjs
    if (filePath === 'META-INF/encryption.xml') continue;

    const bytes = await entry.async('uint8array');
    const needsDecrypt =
      encryptedPaths.has(filePath) || encryptedPaths.has(decodeURIComponent(filePath));

    if (needsDecrypt) {
      if (bytes.length < 32) throw new Error(`File terenkripsi ${filePath} terlalu pendek`);
      const iv = bytes.slice(0, 16);
      const ct = bytes.slice(16);
      const plain = aesCbcDecryptW3C(contentKey, iv, ct);
      outZip.file(filePath, plain, { compression: 'STORE' });
    } else {
      outZip.file(filePath, bytes, { compression: 'STORE' });
    }
  }

  return outZip.generateAsync({ type: 'uint8array' });
}
