...
type Tab = 'users' | 'books' | 'transactions' | 'publishers';
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.tabs}>
        {(['users', 'publishers', 'books', 'transactions'] as Tab[]).map(t => (
          <button
            key={t}
            className={`${styles.tab} ${tab === t ? styles.tabActive : ''}`}
            onClick={() => setTab(t)}
          >
            {t === 'users' ? 'Users' : t === 'publishers' ? 'Approval Publisher' : t === 'books' ? 'Buku' : 'Transaksi'}
          </button>
        ))}
      </div>
