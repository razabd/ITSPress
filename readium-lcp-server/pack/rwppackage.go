...
func BuildRPFFromPDF(inputPath, packagePath, coverPath string, pdfNoMeta bool) (RWPInfo, error) {
	...
	// add the cover image to the package only if coverPath is provided
	// (coverPath is empty on nofitz Windows builds where cover extraction is disabled)
	if coverPath != "" {
		writer, err = zipWriter.CreateHeader(&zip.FileHeader{
			Name:   "cover.jpg",
			Method: zip.Store,
	...
	// only add cover resource to manifest if cover was actually created
	if coverPath != "" {
